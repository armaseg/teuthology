...
def task(ctx, config):
    ...
    for role, role_config in config.iteritems():
        ...
        if role_config.get('rpm') or role_config.get('deb'):
        ...
        elif role_config.get("brew", None):
            # installing a kernel from brewhub
            build_id = role_config.get("brew")
            if system_type != "rhel":
                msg = (
                    "Installing a kernel from brew is only supported "
                    "on rhel systems. System type is {system_type}."
                )
            ...
            if proc.exitstatus == 0:
                ...
                # take the __repr__ and make it python dict again
